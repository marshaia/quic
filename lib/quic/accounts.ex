defmodule Quic.Accounts do
  ...
  defp author_email_multi(author, email, context) do
    ...
    |> Ecto.Multi.delete_all(:tokens, AuthorToken.by_author_and_contexts_query(author, [context]))
  end
  ...
  def update_author_password(author, password, attrs) do
    ...
    Ecto.Multi.new()
    |> Ecto.Multi.update(:author, changeset)
    |> Ecto.Multi.delete_all(:tokens, AuthorToken.by_author_and_contexts_query(author, :all))
    |> Repo.transaction()
    |> case do
  ...
  def delete_author_session_token(token) do
    Repo.delete_all(AuthorToken.by_token_and_context_query(token, "session"))
    :ok
  end
  ...
  defp confirm_author_multi(author) do
    ...
    |> Ecto.Multi.delete_all(:tokens, AuthorToken.by_author_and_contexts_query(author, ["confirm"]))
  end
