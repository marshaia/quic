defmodule QuicWeb.CoreComponents do
  @moduledoc """
  ...
  def modal(assigns) do
    ...
    >
      <div id={"#{@id}-bg"} class="fixed inset-0 transition-opacity bg-zinc-300/90 dark:bg-slate-900/90" aria-hidden="true" />
      <div
        ...
        <div class="flex items-center justify-center min-h-full">
          <div class={"w-full #{if @width_class === "", do: "max-w-3xl", else: @width_class}  p-4 sm:p-6 lg:py-8"}>
            <.focus_wrap
              ...
              class="relative hidden transition bg-[var(--background-card)] shadow-lg shadow-zinc-700/10 ring-zinc-700/10 rounded-2xl p-14 ring-1"
            >
  ...
  def input(%{type: "checkbox"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center gap-4 text-sm leading-6 text-[var(--primary-color-text)]">
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="block w-full mt-2 bg-[var(--background-card)] border border-gray-300 rounded-md shadow-sm focus:border-zinc-400 focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
  ...
  def input(assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <input
        ...
        class={[
          "mt-2 block w-full rounded-lg text-[var(--primary-color-text)] focus:ring-0 sm:text-sm sm:leading-6",
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class="block text-sm font-semibold leading-6 text-[var(--primary-color-text)]">
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(QuicWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(QuicWeb.Gettext, "errors", msg, opts)
    end
  end
