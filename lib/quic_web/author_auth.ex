defmodule QuicWeb.AuthorAuth do
  use QuicWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_quic_web_author_remember_me"
  ...
  def log_in_author(conn, author, params \\ %{}) do
    ...
    |> redirect(to: ~p"/authors") #(to: author_return_to || signed_in_path(conn))
  end
  ...
  def log_out_author(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      QuicWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_author:
      ...
      defmodule QuicWeb.PageLive do
        use QuicWeb, :live_view

        on_mount {QuicWeb.AuthorAuth, :mount_current_author}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{QuicWeb.AuthorAuth, :ensure_authenticated}] do
  ...
  defp mount_current_author(socket, session) do
    Phoenix.Component.assign_new(socket, :current_author, fn ->
