defmodule QuicWeb.ParticipantLive.WaitRoom do
  ...
  def render(assigns) do
    ~H"""
    <div class="flex flex-col items-center justify-center w-full h-full gap-4 p-4 sm:px-6 lg:px-20">
      <h3 class="mb-10 font-normal text-center">Waiting for session <span class="font-medium text-gradient"><%= @session_code %></span> to start</h3>

      <div role="status">
        <svg aria-hidden="true" class="w-16 h-16 animate-spin text-[var(--border)] fill-[var(--primary-color)]" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
