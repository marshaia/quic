defmodule Quic.AccountsTest do
  use Quic.DataCase

  alias Quic.Accounts

  import Quic.AccountsFixtures
  alias Quic.Accounts.{Author, AuthorToken}
  ...
  describe "get_author!/1" do
    test "raises if id is invalid" do
      assert_raise Ecto.NoResultsError, fn ->
        Accounts.get_author!("11111111-1111-1111-1111-111111111111")
      end
    end
  ...
  describe "apply_author_email/3" do
    ...
    test "validates email uniqueness", %{author: author} do
      ...
      {:error, changeset} = Accounts.apply_author_email(author, password, %{email: email})
  ...
  describe "deliver_author_update_email_instructions/3" do
    setup do
      %{author: author_fixture()}
    end
    ...
    test "sends token through notification", %{author: author} do
      token =
        extract_author_token(fn url ->
          Accounts.deliver_author_update_email_instructions(author, "current@example.com", url)
        end)
  ...
  describe "update_author_email/2" do
    setup do
      ...
      token =
        extract_author_token(fn url ->
          Accounts.deliver_author_update_email_instructions(%{author | email: email}, author.email, url)
        end)
  ...
  describe "delete_author_session_token/1" do
    test "deletes the token" do
      author = author_fixture()
      token = Accounts.generate_author_session_token(author)
      assert Accounts.delete_author_session_token(token) == :ok
      refute Accounts.get_author_by_session_token(token)
    end
  end
  ...
  describe "inspect/2 for the Author module" do
