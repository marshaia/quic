defmodule QuicWeb.AuthorAuthTest do
  use QuicWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Quic.Accounts
  alias QuicWeb.AuthorAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, QuicWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_author/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      QuicWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :ensure_authenticated" do
    test "authenticates current_author based on a valid author_token", %{conn: conn, author: author} do
      author_token = Accounts.generate_author_session_token(author)
      session = conn |> put_session(:author_token, author_token) |> get_session()
