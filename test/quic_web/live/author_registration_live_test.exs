defmodule QuicWeb.AuthorRegistrationLiveTest do
  ...
  describe "register author" do
    test "creates account and logs the author in", %{conn: conn} do
      ...
      form = form(lv, "#registration_form", author: valid_author_attributes(email: email))
      render_submit(form)
    ...
    test "renders errors for duplicated email", %{conn: conn} do
      ...
      author = author_fixture(%{email: "test@email.com"})

      result =
        lv
        |> form("#registration_form",
          author: %{"email" => author.email, "password" => "valid_password"}
        )
        |> render_submit()
