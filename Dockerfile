...
RUN apt-get update -y && apt-get -f install -y && \
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/quic ./

RUN chown nobody:root /app/bin/server && chmod +x /app/bin/server && chmod +x /app/bin/migrate
USER nobody
